...
VZ=/usr/sbin/vzctl
VM_ID=666
VM_PATH=/var/lib/vz/private/$VM_ID
NAME_SERVER=`cat /etc/resolv.conf | grep nameserver | cut -d " " -f 2 | head -n 1`

$VZ create $VM_ID --ostemplate debian-7.0-i386-minimal --config basic --hostname gtta --ipadd 192.168.66.66
$VZ set $VM_ID --cpuunits 1000 --cpulimit 50 --nameserver $NAME_SERVER --diskspace 5G --privvmpages 1G --noatime yes --save
$VZ start $VM_ID
sleep 60

$VZ exec2 $VM_ID "echo \"deb http://ftp.de.debian.org/debian wheezy main contrib non-free\" > /etc/apt/sources.list"
$VZ exec2 $VM_ID "echo \"deb http://security.debian.org/ wheezy/updates main contrib non-free\" >> /etc/apt/sources.list"
$VZ exec2 $VM_ID "DEBIAN_FRONTEND=noninteractive apt-get -y update"
mkdir -p $VM_PATH/opt/gtta/scripts/lib $VM_PATH/opt/gtta/files
chmod -R 0777 $VM_PATH/opt/gtta/scripts $VM_PATH/opt/gtta/files
cp $VERSION_DIR/tools/run_script.py $VM_PATH/opt/gtta
